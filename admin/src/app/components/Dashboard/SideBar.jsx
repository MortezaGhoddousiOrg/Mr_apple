...
export default function SideBar({ setRendered }) {
    ...
    const handleClick = (item) => {
        ...
        } else if (item === "محصولات") {
            setRendered(<Products />);
        } else if (item === "کاربران") {
            setRendered(<Users />);
        } else if (item === "سفارشات") {
            setRendered(<Orders />);
        }
    };

    return (
        <aside className="fixed right-0 top-16 h-[calc(100vh-4rem)] w-64 bg-gradient-to-b from-gray-900 to-gray-800 shadow-xl overflow-y-auto">
            ...
            <div className="flex flex-col items-center py-6 px-4 border-b border-gray-700">
                ...
                <p className="text-gray-400 text-sm mt-1 px-3 py-1 bg-gray-700 rounded-full">
                    {user.role}
                </p>
            </div>
