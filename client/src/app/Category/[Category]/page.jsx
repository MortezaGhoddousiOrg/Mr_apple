...
export default function Category() {
  ...
  if (!Category || Category.length === 0) {
    return (
      <div className="category-body">
        ...
        <h2>هیچ محصولی برای این دسته `بندی یافت نشد.</h2>
      </div>
    );
  }
  ...
  const filter = product.filter((title) => title.titleEn == Category);
        ...
        <aside className={style.category} key={index}>
          <header className={style.categoryHeader}>
            <div className={style.categoryBox}>
              ...
              <p className={style.categoryDescription}>{item.p}</p>
            ...
            <CategoryTabFeature  Tab={item.categoryTab} onClick={() => router.push(`${item.cate}`)}/>
        </aside>
      ))}
    </div>
  );
